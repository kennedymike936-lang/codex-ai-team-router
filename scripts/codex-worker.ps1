...
function Write-WorkerProtocolEvent {
  param(
    [string]$Path,
    [string]$Type,
    [string]$TaskId,
  ...
  if (-not [string]::IsNullOrWhiteSpace($Outcome)) { $event.outcome = $Outcome }
...
$protocolPath = Join-Path $runDir "protocol-events.jsonl"

Write-WorkerProtocolEvent -Path $protocolPath -Type "worker.started" -TaskId $TaskId -Worker $Worker -Attempt $Attempt
...
$emptyResultPattern = '(?i)completed without (?:a )?text result|produced no result file|empty (?:output|result)|no (?:text )?(?:output|result)'
$blockedResultPattern = '(?i)\b(blocked|cannot|unable|denied|permission)\b|无法|不能|拒绝|权限'
$workerOutcome = if ($workerExitCode -ne 0) {
  "failed"
} elseif ($summaryText -match $blockedResultPattern) {
  "blocked"
} elseif ($changedFiles.Count -gt 0) {
  "changed"
} elseif ([string]::IsNullOrWhiteSpace($summaryText) -or $summaryText -match $emptyResultPattern) {
  "failed"
} else {
  "verified_noop"
}
$workerStatus = $(if ($workerOutcome -in @("changed", "verified_noop")) { "success" } else { "failed" })
if ($workerStatus -eq "failed" -and [string]::IsNullOrWhiteSpace($workerError)) {
  $workerError = $(if ($workerOutcome -eq "blocked") { "Worker reported a blocked outcome." } else { "Worker returned no usable result." })
}
$workerResult = [ordered]@{
  ...
  artifacts = [ordered]@{
    ...
    protocol_events = $protocolPath
  }
}
$workerResult | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $workerResultPath -Encoding UTF8
Write-WorkerProtocolEvent -Path $protocolPath -Type "worker.completed" -TaskId $TaskId -Worker $Worker -Attempt $Attempt -Outcome $workerOutcome
