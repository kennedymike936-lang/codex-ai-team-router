...
try {
  ...
  $effectiveValidationProfile = $(if ($ValidationProfile -eq "standard" -and $dependencyFiles.Count -gt 0) { "full" } else { $ValidationProfile })
  $steps = @()
  ...
  $verificationEvidenceMissing = $effectiveValidationProfile -eq "full" -and $sourceFilesChanged.Count -gt 0 -and $verificationSteps.Count -eq 0
  ...
  $handoff = [ordered]@{
    ...
    validation_profile = [ordered]@{
      requested = $ValidationProfile
      effective = $effectiveValidationProfile
    }
    ...
    checks = [ordered]@{
      ...
      verification_evidence = $(if ($verificationEvidenceMissing) { "fail" } elseif ($verificationSteps.Count -gt 0) { "pass" } else { "not_required" })
    ...
    metrics = [ordered]@{
      ...
      validation_profile = $effectiveValidationProfile
    }
    artifacts = [ordered]@{
