...
export function trustedProxyConfig(env = process.env, systemProxyUrl = "") {
  ...
  // An enabled Windows system proxy is an administrator-selected network
  // route, so honor it first by default. Explicit environment proxies retain
  // direct-first fallback behavior unless AI_TEAM_PROXY_MODE overrides it.
  const defaultMode = systemProxyUrl ? "always" : "fallback";
  const requestedMode = String(env.AI_TEAM_PROXY_MODE || defaultMode).trim().toLowerCase();
